#include <memory>
#include <systemc>
#include <ranges>
#include <vector>
#include <csignal>
#include <verilated.h>
#include <verilated_fst_sc.h>
#include "Vexecute.h"
#include "util.hpp"

using namespace sc_core;
using namespace sc_dt;

struct Predictor {
    sc_bv<32> pc_ex { 0 };
    bool rd_ex { 0 };
    sc_bv<5> rd_address_ex { 0 };
    bool alu_mode_ex { 0 };
    sc_bv<32> alu_result_ex { 0 };
    sc_bv<32> rt_data_ex { 0 };
    bool load_ex { 0 };
    bool load_sign_extend_ex { 0 };
    sc_bv<2> load_store_data_size_mode_ex { 0 };
    bool store_ex { 0 };

    void operator==(const std::unique_ptr<Vexecute>& dut) const {
        assert(pc_ex == dut->pc_ex.read());
        assert(rd_ex == dut->rd_ex.read());
        assert(rd_address_ex == dut->rd_address_ex.read());
        assert(alu_mode_ex == dut->alu_mode_ex.read());
        assert(alu_result_ex == dut->alu_result_ex.read());
        assert(rt_data_ex == dut->rt_data_ex.read());
        assert(load_ex == dut->load_ex.read());
        assert(load_sign_extend_ex == dut->load_sign_extend_ex.read());
        assert(load_store_data_size_mode_ex == dut->load_store_data_size_mode_ex.read());
        assert(store_ex == dut->store_ex.read());
    }
};

const Predictor predictor[] {
    {
        // add      $25,$15,$2
        .pc_ex = 256,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115 + 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // addu     $25,$15,$2
        .pc_ex = 260,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115 + 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // sub      $25,$15,$2
        .pc_ex = 264,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115 - 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // subu     $25,$15,$2
        .pc_ex = 268,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115 - 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // and      $25,$15,$2
        .pc_ex = 272,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115 & 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // or       $25,$15,$2
        .pc_ex = 276,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115 | 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // xor      $25,$15,$2
        .pc_ex = 280,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115 ^ 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // nor      $25,$15,$2
        .pc_ex = 284,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = ~(115 | 102),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // sllv     $25,$15,$2
        .pc_ex = 288,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115U << (102U & 0b1'1111U)),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // srlv     $25,$15,$2
        .pc_ex = 292,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (115U >> (102U & 0b1'1111U)),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // srav     $25,$15,$2
        .pc_ex = 296,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = (int32_t(115) >> (102U & 0b1'1111U)),
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // slt      $25,$15,$2
        .pc_ex = 300,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // sltu     $25,$15,$2
        .pc_ex = 304,
        .rd_ex = 1,
        .rd_address_ex = 25,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 102,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // sll      $15,$2,0x1e
        .pc_ex = 308,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 << 0x1e),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // srl      $15,$2,0x1e
        .pc_ex = 312,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 >> 0x1e),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // sra      $15,$2,0x1e
        .pc_ex = 316,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (static_cast<int32_t>(102) >> (0x1e)),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // addi     $15,$2,-32000
        .pc_ex = 320,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 + (-32000)),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // addiu    $15,$2,-32000
        .pc_ex = 324,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 + (-32000)),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // slti     $15,$2,-32000
        .pc_ex = 328,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 < -32000),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // sltiu    $15,$2,-32000
        .pc_ex = 332,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (uint16_t(102) < uint16_t(-32000)),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // andi     $15,$2,0x7d00
        .pc_ex = 336,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 & 0x7d00),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // ori     $15,$2,0x7d00
        .pc_ex = 340,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 | 0x7d00),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // xori     $15,$2,0x7d00
        .pc_ex = 344,
        .rd_ex = 1,
        .rd_address_ex = 15,
        .alu_mode_ex = 1,
        .alu_result_ex = (102 ^ 0x7d00),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // lui      $14,0x7d00
        .pc_ex = 348,
        .rd_ex = 1,
        .rd_address_ex = 14,
        .alu_mode_ex = 1,
        .alu_result_ex = (0x7d00 << 16),
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // lb       $2,4($10)
        .pc_ex = 352,
        .rd_ex = 1,
        .rd_address_ex = 2,
        .alu_mode_ex = 0,
        .alu_result_ex = 110 + 4,
        .rt_data_ex = 0,
        .load_ex = 1,
        .load_sign_extend_ex = 1,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_BYTE,
        .store_ex = 0,
    },
    {
        // lbu      $14,-4($31)
        .pc_ex = 356,
        .rd_ex = 1,
        .rd_address_ex = 14,
        .alu_mode_ex = 0,
        .alu_result_ex = 131 + (-4),
        .rt_data_ex = 0,
        .load_ex = 1,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_BYTE,
        .store_ex = 0,
    },
    {
        // lh       $14,4($31)
        .pc_ex = 360,
        .rd_ex = 1,
        .rd_address_ex = 14,
        .alu_mode_ex = 0,
        .alu_result_ex = 131 + (4),
        .rt_data_ex = 0,
        .load_ex = 1,
        .load_sign_extend_ex = 1,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_HALF_WORD,
        .store_ex = 0,
    },
    {
        // lhu      $14,-4($31)
        .pc_ex = 364,
        .rd_ex = 1,
        .rd_address_ex = 14,
        .alu_mode_ex = 0,
        .alu_result_ex = 131 + (-4),
        .rt_data_ex = 0,
        .load_ex = 1,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_HALF_WORD,
        .store_ex = 0,
    },
    {
        // lw       $14,8($31)
        .pc_ex = 368,
        .rd_ex = 1,
        .rd_address_ex = 14,
        .alu_mode_ex = 0,
        .alu_result_ex = 131 + (8),
        .rt_data_ex = 0,
        .load_ex = 1,
        .load_sign_extend_ex = 1,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_WORD,
        .store_ex = 0,
    },
    {
        // sb       $17,8($31)
        .pc_ex = 372,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 131 + (8),
        .rt_data_ex = 117,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_BYTE,
        .store_ex = 1,
    },
    {
        // sh       $18,-8($31)
        .pc_ex = 376,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 131 + (-8),
        .rt_data_ex = 118,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_HALF_WORD,
        .store_ex = 1,
    },
    {
        // sw       $19,8($31)
        .pc_ex = 380,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 131 + (8),
        .rt_data_ex = 119,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = Decode::LoadStoreDataSizeMode_WORD,
        .store_ex = 1,
    },
    {
        // beq	    $14,$10,1 <main+0x10>
        .pc_ex = 384,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 110,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // bne	    $10,$10,-3 <main+0x8>
        .pc_ex = 388,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 110,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // blez	$14,1 <main+0x20>
        .pc_ex = 392,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // bgtz     $14,-3 <main+0x128>
        .pc_ex = 396,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // bltz     $14,5 <main+0x140>
        .pc_ex = 400,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // bgez     $11,-7 <main+0x138>
        .pc_ex = 404,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // bltzal   $11,5 <main+0x138>
        .pc_ex = 408,
        .rd_ex = 0,
        .rd_address_ex = 31,
        .alu_mode_ex = 1,
        .alu_result_ex = 408 + 4 + 4,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // bgezal   $1,-9 <main+0x130>
        .pc_ex = 412,
        .rd_ex = 0,
        .rd_address_ex = 31,
        .alu_mode_ex = 1,
        .alu_result_ex = 412 + 4 + 4,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // j        432 (432 / 4 == 108)
        .pc_ex = 416,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // nop -> sll      $0,$0,0x00
        .pc_ex = 420,
        .rd_ex = 1,
        .rd_address_ex = 0,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // jal      448 (448 / 4 = 112)
        .pc_ex = 432,
        .rd_ex = 1,
        .rd_address_ex = 31,
        .alu_mode_ex = 1,
        .alu_result_ex = 432 + 4 + 4, //??
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // ori	     $10,$0,464
        .pc_ex = 436,
        .rd_ex = 1,
        .rd_address_ex = 10,
        .alu_mode_ex = 1,
        .alu_result_ex = 464,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // jr       $10
        .pc_ex = 448,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // ori	     $10,$0,480
        .pc_ex = 452,
        .rd_ex = 1,
        .rd_address_ex = 10,
        .alu_mode_ex = 1,
        .alu_result_ex = 480,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // jalr     $10
        .pc_ex = 464,
        .rd_ex = 1,
        .rd_address_ex = 31,
        .alu_mode_ex = 1,
        .alu_result_ex = 464 + 4 + 4,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // nop -> sll      $0,$0,0x00
        .pc_ex = 468,
        .rd_ex = 1,
        .rd_address_ex = 0,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // nop -> sll      $0,$0,0x00
        .pc_ex = 480,
        .rd_ex = 1,
        .rd_address_ex = 0,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    // ------------------//------------------ //
    {
        // beq      $0,$0,-1
        .pc_ex = 484,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // nop -> sll      $0,$0,0x00
        .pc_ex = 488,
        .rd_ex = 1,
        .rd_address_ex = 0,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    // ------------------//------------------ //
    {
        // beq      $0,$0,-1
        .pc_ex = 484,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // nop -> sll      $0,$0,0x00
        .pc_ex = 488,
        .rd_ex = 1,
        .rd_address_ex = 0,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    // ------------------//------------------ //
    {
        // beq      $0,$0,-1
        .pc_ex = 484,
        .rd_ex = 0,
        .rd_address_ex = 0,
        .alu_mode_ex = 0,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
    {
        // nop -> sll      $0,$0,0x00
        .pc_ex = 488,
        .rd_ex = 1,
        .rd_address_ex = 0,
        .alu_mode_ex = 1,
        .alu_result_ex = 0,
        .rt_data_ex = 0,
        .load_ex = 0,
        .load_sign_extend_ex = 0,
        .load_store_data_size_mode_ex = 0,
        .store_ex = 0,
    },
};

VerilatedFstSc* tfp = nullptr;

int sc_main(int argc, char* argv[]) {
    Verilated::debug(0);
    Verilated::randReset(2);
    Verilated::traceEverOn(true);
    Verilated::commandArgs(argc, argv);
    std::ios::sync_with_stdio();

    // inputs
    sc_clock clk{ "clk", sc_time { 10.0, SC_NS }, 0.5, sc_time { 3.0, SC_NS } };
    sc_signal<bool> nrst;
    const uint8_t ROM[] {
        // nop; sll $0, $0, 0 type instructions for writeback check
        ,

        // asm("sw $0,  10($1)"); type instructions for writeback check
        , // add      $25,$15,$2
        0x01,0xe2,0xc8,0x21, // addu     $25,$15,$2
        0x01,0xe2,0xc8,0x22, // sub      $25,$15,$2
        0x01,0xe2,0xc8,0x23, // subu     $25,$15,$2
        0x01,0xe2,0xc8,0x24, // and      $25,$15,$2
        0x01,0xe2,0xc8,0x25, // or       $25,$15,$2
        0x01,0xe2,0xc8,0x26, // xor      $25,$15,$2
        0x01,0xe2,0xc8,0x27, // nor      $25,$15,$2
        0x00,0x4f,0xc8,0x04, // sllv     $25,$15,$2
        0x00,0x4f,0xc8,0x06, // srlv     $25,$15,$2
        0x00,0x4f,0xc8,0x07, // srav     $25,$15,$2
        0x01,0xe2,0xc8,0x2a, // slt      $25,$15,$2
        0x01,0xe2,0xc8,0x2b, // sltu     $25,$15,$2
        0x00,0x02,0x7f,0x80, // sll      $15,$2,0x1e
        0x00,0x02,0x7f,0x82, // srl      $15,$2,0x1e
        0x00,0x02,0x7f,0x83, // sra      $15,$2,0x1e
        0x20,0x4f,0x83,0x00, // addi     $15,$2,-32000
        0x24,0x4f,0x83,0x00, // addiu    $15,$2,-32000
        0x28,0x4f,0x83,0x00, // slti     $15,$2,-32000
        0x2c,0x4f,0x83,0x00, // sltiu    $15,$2,-32000
        0x30,0x4f,0x7d,0x00, // andi     $15,$2,0x7d00
        0x34,0x4f,0x7d,0x00, // ori      $15,$2,0x7d00
        0x38,0x4f,0x7d,0x00, // xori     $15,$2,0x7d00
        0x3c,0x0e,0x7d,0x00, // lui      $14,0x7d00
        0x81,0x42,0x00,0x04, // lb       $2,4($10)
        0x93,0xee,0xff,0xfc, // lbu      $14,-4($31)
        0x87,0xee,0x00,0x04, // lh       $14,4($31)
        0x97,0xee,0xff,0xfc, // lhu      $14,-4($31)
        0x8f,0xee,0x00,0x08, // lw       $14,8($31)
        0xa3,0xf1,0x00,0x08, // sb       $17,8(ra)
        0xa7,0xf2,0xff,0xf8, // sh       $18,-8(ra)
        0xaf,0xf3,0x00,0x08, // sw       $19,8(ra)

        0x11,0xca,0x00,0x01, // beq	    $14,$10,1 <main+0x10>
        0x15,0x4a,0xff,0xfd, // bne	    $10,$10,-3 <main+0x8>
        0x19,0xc0,0x00,0x01, // blez	$14,1 <main+0x20>
        0x1c,0x20,0xff,0xfd, // bgtz	$1,-3 <main+0x18>
        0x05,0xc0,0x00,0x05, // bltz	$14,5 <main+0x40>
        0x04,0x21,0xff,0xf9, // bgez	$1,-7 <main+0x18>
        0x05,0x70,0x00,0x05, // bltzal	$11,5 <main+0x50>
        0x04,0x31,0xff,0xf9, // bgezal	$1,-9 <main+0x28>

        0x08,0x00,0x00,0x6c, // j        432 (432 / 4 == 108)
        0x00,0x00,0x00,0x00, // nop
        0x00,0x00,0x00,0x00, // nop
        0x00,0x00,0x00,0x00, // nop

        0x0c,0x00,0x00,0x70, // jal      448 (448 / 4 = 112)
        0x34,0x0a,0x01,0xd0, // ori	     $10,$0,464
        0x00,0x00,0x00,0x00, // nop
        0x00,0x00,0x00,0x00, // nop

        0x01,0x40,0x00,0x08, // jr       $10
        0x34,0x0a,0x01,0xe0, // ori	     $10,$0,480
        0x00,0x00,0x00,0x00, // nop
        0x00,0x00,0x00,0x00, // nop

        0x01,0x40,0xf8,0x09, // jalr     $10
        0x00,0x00,0x00,0x00, // nop
        0x00,0x00,0x00,0x00, // nop
        0x00,0x00,0x00,0x00, // nop

        0x00,0x00,0x00,0x00, // nop; sll $0, $0, 0 to verify branched variables from previous cycle
        0x10,0x00,0xff,0xff, // beq	$0,$0,-1
        0x00,0x00,0x00,0x00, // nop; sll $0, $0, 0 to verify branched variables from previous cycle
    };

    static_assert((sizeof(ROM) > 4) && ((sizeof(ROM) % 4) == 0));
    std::vector<sc_signal<sc_bv<8>>> rom(std::extent_v<std::remove_reference_t<decltype(Vexecute::rom)>>);
    sc_signal<bool> stall;
    sc_signal<bool> rd_wb;
    sc_signal<sc_bv<5>> rd_address_wb;
    sc_signal<sc_bv<32>> rd_data_wb;

    // outputs
    sc_signal<bool> rd_ex;
    sc_signal<bool> alu_mode_ex;
    sc_signal<bool> load_ex;
    sc_signal<bool> load_sign_extend_ex;
    sc_signal<bool> store_ex;

    sc_signal<sc_bv<32>> pc_ex;
    sc_signal<sc_bv<5>> rd_address_ex;
    sc_signal<sc_bv<32>> alu_result_ex;
    sc_signal<sc_bv<32>> rt_data_ex;
    sc_signal<sc_bv<2>> load_store_data_size_mode_ex;
    std::vector<sc_signal<sc_bv<32>>> reg_file(std::extent_v<std::remove_reference_t<decltype(Vexecute::reg_file)>>);

    const std::unique_ptr<Vexecute> dut{new Vexecute{"execute_context"}};

    // inputs
    dut->clk(clk);
    dut->nrst(nrst);
    for(const auto& [port, sig]: std::views::zip(dut->rom, rom)) {
        port(sig);
    }
    dut->stall(stall);
    dut->rd_wb(rd_wb);
    dut->rd_address_wb(rd_address_wb);
    dut->rd_data_wb(rd_data_wb);

    // outputs
    dut->rd_ex(rd_ex);
    dut->alu_mode_ex(alu_mode_ex);
    dut->load_ex(load_ex);
    dut->load_sign_extend_ex(load_sign_extend_ex);
    dut->store_ex(store_ex);

    dut->pc_ex(pc_ex);
    dut->rd_address_ex(rd_address_ex);
    dut->alu_result_ex(alu_result_ex);
    dut->rt_data_ex(rt_data_ex);
    dut->load_store_data_size_mode_ex(load_store_data_size_mode_ex);
    for(const auto& [port, sig]: std::views::zip(dut->reg_file, reg_file)) {
        port(sig);
    }


    nrst = 1;
    stall = 0;
    for(const auto& [data, sig]: std::views::zip(ROM, rom)) {
        sig = data;
    }
    rd_wb = 0;
    rd_address_wb = 0;
    rd_data_wb = 0;

    sc_start(SC_ZERO_TIME);
    Verilated::mkdir("logs");
    tfp = new VerilatedFstSc;
    dut->trace(tfp, 99);
    tfp->open("logs/execute_tb.fst");
    std::signal(SIGABRT, [](int signal) { if(tfp) { tfp->flush(); tfp->close(); }});

    // reset
    sc_start(1, SC_NS);
    nrst = 0;
    sc_start(1, SC_NS);
    Predictor{} == dut;
    nrst = 1;
    sc_start(1, SC_NS);

    // register file writeback with sll $0, $0, 0 or nops
    for(const uint32_t i: std::views::iota(0U, Constants::REG_COUNT)) {
        rd_wb = 1;
        rd_address_wb = i + 0;
        rd_data_wb = (i == 1) ? (-(i + 100)) : (i + 100);
        sc_start(5, SC_NS);
        sc_start(5, SC_NS);
    }
    rd_wb = 0;
    rd_address_wb = 0;
    rd_data_wb = 0;

    sc_start(5, SC_NS);
    sc_start(5, SC_NS);
    sc_start(5, SC_NS);
    sc_start(5, SC_NS);

    // register file read with sw rt, imm(rs)
    for(const uint32_t i: std::views::iota(0U, Constants::REG_COUNT)) {
        sc_start(5, SC_NS);
        assert(dut->store_ex.read() == 1);
        if(i == 0) {
            assert(dut->rt_data_ex.read() == 0);
        } else {
            assert(dut->rt_data_ex.read() == ((i == 1) ? (-(i + 100)) : (i + 100)));
        }
        
        if(i == Constants::REG_COUNT - 1) {
            assert(dut->alu_result_ex.read() == 0 + 10);
        } else {
            assert(dut->alu_result_ex.read() == ((i + 1 == 1) ? (-(i + 1 + 100) + 10) : ((i + 1 + 100) + 10)));
        }
        sc_start(5, SC_NS);
    }

    for(const Predictor& i: predictor) {
        sc_start(5, SC_NS);
        i == dut;
        sc_start(5, SC_NS);
    }

    if(tfp) { tfp->flush(); tfp->close(); }
    dut->final();
    return 0;
}